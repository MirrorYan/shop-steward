<template>
  <div class="page-purchase">
    ...
    <div class="result-container">
      <InfoCard v-for="item in list" :key="item.returnId">
        <h3>{{ item.productName }}
          <br><span class="info-number">No.{{ item.productNo }}</span></h3>
        <span class="info-date">{{ item.returnDate }}</span>
        ...
        <div class="info-list-item" v-if="item.valueList">
          ...
          <div class="info-list-right">
            <p class="info-price"><small>¥</small>{{ item.totalPrice }}</p>
            <p class="info-quantity"><small>x</small>{{ item.returnNums }}</p>
          </div>
        </div>
      </InfoCard>
    </div>
...
import { getReturnedList } from '@/network/api';
